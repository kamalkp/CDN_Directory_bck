...
//	out, _ := exec.Command("/root/CDN/ssl_cert_generate.sh", *domainNamePtr).Output()
//	fmt.Println(out)
			...
			for looper := j; looper <= k; looper++ {
				...
				if l < k {
					...
					if !strings.Contains(lines[l+1], "#"){
						lines[l+1] = "\t\ttestcookie off ; "	//this section of the code needs tuning - the entire nested loop
					}
